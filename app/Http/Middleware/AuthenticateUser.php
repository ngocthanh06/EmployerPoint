...
class AuthenticateUser
{
    ...
    public function handle(Request $request, Closure $next)
    {
        if (!$token = JWTAuth::setRequest($request)->getToken()) {
            return api_errors(config('code.auth.token_absent'), [], 200, ['guard' => config('auth.guard_type.user')]);
        }
    ...
    {
        return api_errors(config('code.auth.token_invalid'), [], 200, ['guard' => config('auth.guard_type.user')]);
    }

    private function responseTokenBlacklisted()
    {
        return api_errors(config('code.auth.token_blacklisted'), [], 200, ['guard' => config('auth.guard_type.user')]);
    }

    private function responseUserNotFound()
    {
        return api_errors(config('code.user.user_not_found'), [], 200, ['guard' => config('auth.guard_type.user')]);
    }

    private function authenticateByToken($token)
    {
        Auth::guard('user')->authenticate($token);
    }
}
